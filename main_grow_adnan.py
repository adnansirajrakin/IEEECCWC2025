...
def main():
    ...
    for name,m in model.named_modules():
        if isinstance(m,BinarizeConv2d):
            ...
            num_one = m.masked_bw.eq(1).sum()
            all_one += num_one.item()
            all_num += m.masked_bw.numel()
            density = num_one.item()/m.masked_bw.numel()
            if m.mask.grad is not None:
                print('Layer: {},  Density:{:2.4f}, Mean:{:2.4f}, Max:{:2.4f}, Min:{:2.4f}, Grad Mean:{:2.6f}, Max:{:2.4f}, Min:{:2.4f}'.format(idx, density, 
                                m.mask.mean().item(), m.mask.max().item(),m.mask.min().item(),
                                m.mask.grad.mean().item(), m.mask.grad.max().item(),m.mask.grad.min().item()))
            else:
                print('Layer: {},  Density:{:2.4f}, Mean:{:2.4f}, Max:{:2.4f}, Min:{:2.4f}'.format(idx, density, 
                m.mask.mean().item(), m.mask.max().item(),m.mask.min().item()))
            idx = idx + 1
    print('Overall Density:{:2.4f}'.format(all_one/all_num))
    if args.evaluate:
    ...
    optimizer = torch.optim.SGD([{'params':model_param,'initial_lr':args.lr},
                                {'params':mask_param,'initial_lr':args.lr, 'weight_decay': 0.}],
                                args.lr,
    ...
    for epoch in range(args.start_epoch+1, args.epochs):
        ...
        # increase Beta in here
        if epoch < 800:
            for name,m in model.named_modules():
                if isinstance(m,BinarizeConv2d):
                # T_schel = tempurature_scheduler(m.Beta)
                    indices = (m.masked_bw == 1).nonzero(as_tuple=True)
                    m.Beta[indices] *= 100**(1./80.)
        ...
        if is_best:
            torch.save(model.state_dict(), './models/gr.pkl')
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        ii=0
        ## Li: computing L1 penalty
        #masks = [m.mask * m.Beta for m in model.BinarizeConv2d]
       
        #sig_sum = sum(torch.sigmoid(m).sum() for m in masks)
        ...
        if training:
            ...
            optimizer.zero_grad()
            loss.backward()
            # Adnan: we need to mask the grad of part mask to be zero 
            # example code:
            '''for idx, module in enumerate(model.modules()):
                if isinstance(module, (BinarizeConv2d)):
                    module.masked_bw.grad.data[:] = 0'''
    ...
    ## Li: Performing scheduling upto 900 epoch

    
    
    return losses.avg, top1.avg, top5.avg

class tempurature_scheduler():

    def __init__(self, temp):
        # temp is beta 
        self.temp = temp
        self.counter = torch.ones(self.temp.size())


    def reinitialize(self, indices):

        # indices is kernel size 1st dim
        # indices is pruned indices
        
        self.temp.data[indices] = torch.ones(self.temp.data[indices].size()).cuda()
    ...
    def step(self, indices, update=False, factor=100**(1./80.)):

        # indices are activated ones

        if update:

            self.temp.data[indices] *= factor
